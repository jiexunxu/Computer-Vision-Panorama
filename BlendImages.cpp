...
{	
    ...
    for (int y = bb_min_y; y <= bb_max_y; y++) {
		...
		//	weight=min(x_src/blendWidth, (img.Shape().width-x_src)/blendWidth);
			// *** END TODO ***	
...
static void NormalizeBlend(CFloatImage& acc, CByteImage& img)
{
	...
	for(int x=0;x<acc.Shape().width;x++){
		for(int y=0;y<acc.Shape().height;y++){			
			float alpha=acc.Pixel(x, y, 3);
			if(alpha==0){
				alpha=1.0;
				img.Pixel(x, y, 0)=acc.Pixel(x, y, 0)/alpha;
				img.Pixel(x, y, 1)=acc.Pixel(x, y, 1)/alpha;
				img.Pixel(x, y, 2)=acc.Pixel(x, y, 2)/alpha;
				continue;
			}
			...
			img.Pixel(x, y, 2)=acc.Pixel(x, y, 2)/alpha;
		//	img.Pixel(x, y, 3)=1;
		/*	uchar red=(uchar)(255.0*acc.Pixel(x, y, 0)/alpha);
			uchar green=(uchar)(255.0*acc.Pixel(x, y, 1)/alpha);
			uchar blue=(uchar)(255.0*acc.Pixel(x, y, 2)/alpha);
			img.Pixel(x, y, 0)=red;
			img.Pixel(x, y, 1)=green;
			img.Pixel(x, y, 2)=blue;
			img.Pixel(x, y, 3)=(uchar)255;
			if(debug_ct<10){
			cout<<acc.Pixel(x, y, 0)<<" "<<acc.Pixel(x, y, 1)<<" "<<acc.Pixel(x, y, 2)<<" "<<acc.Pixel(x, y, 3)<<" ";
			cout<<(int)img.Pixel(x, y, 0)<<" "<<(int)img.Pixel(x, y, 1)<<" "<<(int)img.Pixel(x, y, 2)<<" "<<(int)img.Pixel(x, y, 3)<<"\n";
			debug_ct++;
			}*/
		//	debug<<x<<" "<<y<<" "<<(int)red<<" "<<(int)green<<" "<<(int)blue<<"\n";
		}
	}
	// *** END TODO ***
}
...
CByteImage BlendImages(CImagePositionV& ipv, float blendWidth)
{
    ...
    for (i = 0; i < n; i++)
    {
		...
		// *** BEGIN TODO #1 ***
		// add some code here to update min_x, ..., max_y to compute the bounding box for the mosaic collection
		for(int j=0;j<4;j++){
			if(corners[j][0]<min_x)
				min_x=(float)corners[j][0];
			if(corners[j][0]>max_x)
				max_x=(float)corners[j][0];
			if(corners[j][1]<min_y)
				min_y=(float)corners[j][1];
			if(corners[j][1]>max_y)
				max_y=(float)corners[j][1];
		}
/*float cand_min_x=(float)min(corners[0][0], corners[2][0]);
		if(cand_min_x<min_x)
			min_x=cand_min_x;

		float cand_max_x=(float)max(corners[1][0], corners[3][0]);
		if(cand_max_x>max_x)
			max_x=cand_max_x;

		float cand_min_y=(float)min(corners[0][1], corners[1][1]);
		if(cand_min_y<min_y)
			min_y=cand_min_y;

		float cand_max_y=(float)max(corners[2][1], corners[3][1]);
		if(cand_max_y>max_y)
			max_y=cand_max_y;*/
		// *** END TODO #1 ***
    }
	cout<<"minx="<<min_x<<" maxx="<<max_x<<" miny="<<min_y<<" maxy="<<max_y<<"\n";
...
//	A[0][0]=1;A[1][1]=1;A[2][2]=1;
	A=CTransform3x3::Translation(0, 0);
	// *** END TODO #2 ***
